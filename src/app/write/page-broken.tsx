...
export default function WritePageEnhanced() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (creditsError) {
      alert('额度检查失败，请刷新页面重试');
      return;
    }
    
    if (!hasCredits) {
      alert('额度不足！请升级到专业版或等待下月重置。');
      router.push('/upgrade');
      return;
    }

    // 如果是免费用户且额度不足，显示警告
    if (subscriptionTier === 'free' && currentCredits <= 0) {
      alert('免费额度已用完！请升级到专业版继续使用。');
      router.push('/upgrade');
      return;
    }
    ...
    try {
      ...
      if (response.success) {
        ...
        // 保存到历史记录
        if (response.data) {
          // 这里需要saveToHistory函数，稍后添加
          console.log('保存历史记录:', response.data.generationId)
        }
        
        // 如果是普通写作模式，保存当前设置
        if (!isInspirationMode) {
          // 这里需要saveSettings函数，稍后添加
          console.log('保存用户设置')
        }
  ...
  {/* ===== 明显的额度显示区域 ===== */}
  <div className="mb-8 p-6 bg-gradient-to-br from-blue-500 to-purple-600 rounded-2xl shadow-xl text-white">
      ...
      <div className="bg-white/10 p-4 rounded-xl backdrop-blur-sm">
        ...
        {!creditsLoading && (
          <div className="text-sm text-blue-200 mt-2">
            总购买额度: {totalPurchasedCredits || 0} 次
          </div>
        )}
      </div>
      ...
      <div className="bg-white/10 p-4 rounded-xl backdrop-blur-sm">
        ...
        <div className="text-xl font-bold">
          {creditsLoading ? (
            ...
            <div className="text-red-300">❌ 错误: {creditsError}</div>
          ) : hasCredits ? (
        ...
        <div className="text-sm text-blue-200 mt-2">
          {creditsLoading ? '正在检查...' : 
           creditsError ? '请检查网络连接' :
           hasCredits ? '可以开始写作' : '请购买套餐'}
        </div>
      </div>
    </div>
    
    {/* 使用进度条 */}
    {!creditsLoading && totalPurchasedCredits && totalPurchasedCredits > 0 && (
      <div className="mt-6">
        <div className="flex justify-between text-sm text-blue-200 mb-2">
          <span>使用进度</span>
          <span>{totalPurchasedCredits - currentCredits}/{totalPurchasedCredits} 次</span>
        </div>
        <div className="w-full bg-white/20 rounded-full h-3">
          <div 
            className="bg-gradient-to-r from-green-400 to-cyan-400 h-3 rounded-full transition-all duration-1000"
            style={{ width: `${((totalPurchasedCredits - currentCredits) / totalPurchasedCredits) * 100}%` }}
          ></div>
        </div>
    ...
    <div className="mt-6 flex gap-4">
      <button
        onClick={() => {
          console.log('额度状态:', { hasCredits, currentCredits, subscriptionTier, creditsLoading, creditsError });
    ...
    {process.env.NODE_ENV === 'development' && (
      <div className="mt-6 p-4 bg-black/30 rounded-lg">
        <div className="text-sm font-mono text-blue-200">
          <div>📊 调试信息:</div>
          <div>• creditsLoading: {creditsLoading.toString()}</div>
          <div>• hasCredits: {hasCredits.toString()}</div>
          <div>• currentCredits: {currentCredits}</div>
          <div>• subscriptionTier: {subscriptionTier}</div>
          <div>• creditsError: {creditsError || '无'}</div>
        </div>
      </div>
    )}
  </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {/* 订阅等级 */}
      <div className="bg-white/10 p-4 rounded-xl backdrop-blur-sm">
        <div className="text-sm text-blue-200 mb-1">订阅等级</div>
        <div className="text-3xl font-bold">
          {creditsLoading ? (
            <div className="animate-pulse">加载中...</div>
      ...
      <div className="bg-white/10 p-4 rounded-xl backdrop-blur-sm">
        ...
        <div className="text-3xl font-bold">
          {creditsLoading ? (
            ...
            <span className="text-green-300">无限</span>
          ) : (
            <span>{currentCredits} 次</span>
          )}
        </div>
        {!creditsLoading && subscriptionTier === 'free' && (
          <div className="text-sm text-blue-200 mt-2">
            本月已使用 {10 - currentCredits}/10 次
          </div>
        )}
      </div>
      ...
      {/* 状态指示 */}
      <div className="bg-white/10 p-4 rounded-xl backdrop-blur-sm">
        <div className="text-sm text-blue-200 mb-1">系统状态</div>
        <div className="text-xl font-bold">
          {creditsLoading ? (
            <div className="flex items-center gap-2">
              <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></div>
    ...
    {!creditsLoading && subscriptionTier === 'free' && (
      <div className="mt-6">
        <div className="flex justify-between text-sm text-blue-200 mb-2">
          <span>本月使用进度</span>
          <span>{10 - currentCredits}/10 次</span>
        </div>
        <div className="w-full bg-white/20 rounded-full h-3">
          <div 
            className="bg-gradient-to-r from-green-400 to-cyan-400 h-3 rounded-full transition-all duration-1000"
            style={{ width: `${((10 - currentCredits) / 10) * 100}%` }}
          ></div>
        </div>
        <div className="flex justify-between text-xs text-blue-200 mt-2">
          <span>0次</span>
    ...
    <div className="mt-6 flex gap-4">
      <button
        onClick={() => {
          console.log('额度状态:', { hasCredits, currentCredits, subscriptionTier, creditsLoading, creditsError });
          alert(`额度状态:\n- 订阅等级: ${subscriptionTier}\n- 剩余额度: ${subscriptionTier === 'pro' ? '无限' : currentCredits + '次'}\n- 有额度: ${hasCredits ? '是' : '否'}`);
        }}
        className="px-5 py-3 bg-white/20 hover:bg-white/30 rounded-lg font-medium transition-colors"
      >
        显示详情
      ...
      <button
        ...
        className="px-5 py-3 bg-white text-blue-600 hover:bg-blue-50 rounded-lg font-bold transition-colors flex-1"
      >
        {subscriptionTier === 'pro' ? '🎉 您已是专业版' : '🚀 升级到专业版'}
      </button>
    </div>
                    ...
                    >
                      {loading ? (
                        ...
                        <div className="flex items-center justify-center">
                          ...
                          {isInspirationMode ? (isPlusMode ? '创意组合' : '生成灵感') : '开始AI写作'}
                        </div>
                      )}
                    </button>
